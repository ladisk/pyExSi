...
def get_nonstationarity_data(show=False):
    ...
    nongausian_signal = es.stationary_nongaussian_signal(N, PSD, fs, k_u=k_u_target, rg=rg)
    #calculate kurtosis
    ...
    delta_m_list = np.arange(.1,2.1,.25) 
    p_list = np.arange(.1,2.1,.25)
    #get signal 
    nongausian_nonstationary_signal_psd = es.nonstationary_signal(N,PSD,fs,k_u=k_u_target,modulating_signal=('PSD',PSD_modulating),
                                                            param1_list=delta_m_list,param2_list=p_list,seed=seed)
    #calculate kurtosis 
    k_u_nonstationary_nongaussian_psd = es.get_kurtosis(nongausian_nonstationary_signal_psd)
    ...
    #get signal 
    nongausian_nonsttaionary_signal_beta = es.nonstationary_signal(N,PSD,fs,k_u=k_u_target,modulating_signal=('CSI',delta_n),
                                                            param1_list=alpha_list,param2_list=beta_list,seed=seed)
    #calculate kurtosis 
    k_u_nonstationary_nongaussian_beta = es.get_kurtosis(nongausian_nonsttaionary_signal_beta)
    ...
    if show:
        ...
        plt.plot(t[t_indx],nongausian_nonstationary_signal_psd[t_indx], label = 'non-gaussian non-stationary (psd)')
...
def get_signals_data():
    ...
    #burst random
    amplitude = 5
    burst_random = es.burst_random(N=N, A=amplitude, ratio=0.1, distribution='normal', n_bursts=3, rg=rg)
...
if __name__ == "__main__":
    ...
    with open('tests/test_data_nonstationarity_human_should_remove_this_if_you_know_what_are_you_doing.pkl', 'wb') as the_file:
        pickle.dump(test_data, the_file, protocol=4)

    test_data = get_signals_data()
    with open('tests/test_data_signals_human_should_remove_this_if_you_know_what_are_you_doing.pkl', 'wb') as the_file:
        pickle.dump(test_data, the_file, protocol=4)
